...
输入一个点的坐标（以空格分割），能预测他的贫富度，并标出来显示在图上。

### some problems

1. 为什么把n_neighbors设为20？不是说要设成奇数吗？（这里分四类，无所谓了）

2. 画图的时候为什么要搞个底色啊（我感觉底色跟点明明对应不上啊，有啥用啊）（竺佬说画图的时候出了点小问题）
...
plt.contourf(xx, yy, Z, cmap=cmap_light) # 控制图片底色
sns.scatterplot(
    x=X[:, 0],
    y=X[:, 1],
